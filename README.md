...
cp .env.example .env   # edit AWS_REGION / S3_BUCKET / SENDER_EMAIL / BREVO_API_KEY as needed
```
...
This creates the DynamoDB tables (complaints + magic-link tokens) and S3 bucket. For magic-link emails to actually send, sign up for a free [Brevo](https://www.brevo.com) account (300 emails/day, no credit card, no expiry), verify a sender under **Senders, Domains & Dedicated IPs > Senders** (one confirmation-link click, same idea as any email verification), create an API key, and set `SENDER_EMAIL` + `BREVO_API_KEY` in `.env`.

**Note:** without `BREVO_API_KEY` configured (or if a send fails for any reason), `app/email_sender.py` logs the sign-in link server-side instead of emailing it, so sign-in still works — check `docker logs` / server output for the link.
