...
def classify_and_prepare(description: str, location: str) -> str:
    ...
    return json.dumps({"category": category, "priority": priority, "complaint_text": complaint_text})


agent = Agent(model=BedrockModel(), tools=[classify_and_prepare], system_prompt=SYSTEM_PROMPT)
...
def invoke(payload):
    ...
    result = json.loads(text[start:end + 1]) if start != -1 and end != -1 else {}
